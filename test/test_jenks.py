...
class TestJenksNaturalBreaks:
    ...
    def test_basic_classification(self):
        ...
        assert len(breaks) == 3
        assert breaks[0] <= breaks[1] <= breaks[2]
        assert breaks[-1] == np.max(data)
    ...
    def test_low_light_scenario(self):
        ...
        assert len(breaks) == 5
        assert all(0 <= b <= 0.04 for b in breaks)
        assert breaks[-1] == pytest.approx(0.04, rel=1e-6)
    ...
    def test_viirs_like_distribution(self):
        ...
        assert breaks[3] < breaks[4]  # Gap between moderate and high
    ...
    def test_uniform_distribution(self):
        ...
        assert np.std(diffs) < np.mean(diffs) * 0.5  # Reasonably uniform
    ...
    def test_normal_distribution(self):
        ...
        assert abs(breaks[median_break_idx] - 50) < 15
    ...
    def test_nan_handling(self):
        ...
        assert len(breaks) == 3
        assert not np.any(np.isnan(breaks))
    ...
    def test_inf_handling(self):
        ...
        assert len(breaks) == 3
        assert not np.any(np.isinf(breaks))
    ...
    def test_sparse_data(self):
        ...
        assert breaks[0] < 100  # First cluster
        assert 100 <= breaks[1] < 1000  # Second cluster
        assert breaks[2] >= 1000  # Third cluster
...
class TestGoodnessOfVarianceFit:
    ...
    def test_perfect_classification(self):
        """Test GVF with perfectly separated classes."""
        # Three perfectly separated clusters
