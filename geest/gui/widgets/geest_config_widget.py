...
class GeestConfigWidget(QWidget):
    ...
    def __init__(self, config_dict):
        ...
        QgsMessageLog.logMessage(
            f"Initializing GeestConfigWidget with config: {config_dict}",
            "GeestConfigWidget",
            level=Qgis.Info,
        )
        self.create_widgets()
    ...
    def create_widgets(self):
        ...
        QgsMessageLog.logMessage(
            "Calling GeestWidgetFactory.create_widgets",
            "GeestConfigWidget",
            level=Qgis.Info,
        )
        ...
        if not isinstance(widgets_container, QWidget):
            QgsMessageLog.logMessage(
                f"GeestWidgetFactory.create_widgets returned unexpected type:"
        ...
        if widgets_container.layout().count() > 0:
            QgsMessageLog.logMessage(
                f"Adding container with {widgets_container.layout().count()} items",
                "GeestConfigWidget",
                level=Qgis.Info,
            )
        ...
        QgsMessageLog.logMessage(
            self.dump_widget_hierarchy(widgets_container),
            "GeestConfigWidget",
            level=Qgis.Info,
        )
    ...
    def find_and_store_widgets(self, container):
        QgsMessageLog.logMessage(
            "Starting to find and store widgets", "GeestConfigWidget", level=Qgis.Info
        )
        self.recursive_find_and_store_widgets(container)
        QgsMessageLog.logMessage(
            f"Total widgets stored: {len(self.widgets)}",
            "GeestConfigWidget",
            level=Qgis.Info,
        )
    ...
    def recursive_find_and_store_widgets(self, widget, depth=0):
        QgsMessageLog.logMessage(
            "  " * depth + f"Examining widget: {type(widget).__name__}",
            "GeestConfigWidget",
            level=Qgis.Info,
        )
        ...
        if use_key:
            ...
            if isinstance(widget, QRadioButton):
                ...
                QgsMessageLog.logMessage(
                    "  " * depth + f"Stored QRadioButton for key: {use_key}",
                    "GeestConfigWidget",
                    level=Qgis.Info,
                )
            elif isinstance(
                ...
                self.widgets[use_key]["widget"] = widget
                QgsMessageLog.logMessage(
                    "  " * depth + f"Stored multibuffer widget for key: {use_key}",
                    "GeestConfigWidget",
                    level=Qgis.Info,
                )
            elif (
                ...
                QgsMessageLog.logMessage(
                    "  " * depth
                    + f"Stored composite widget (polygon_layer_with_field_selector) "
                    f"for key: {use_key}",
                    "GeestConfigWidget",
                    level=Qgis.Info,
                )
        ...
        QgsMessageLog.logMessage(
            f"Current widgets dictionary: {self.widgets}",
            "GeestConfigWidget",
            level=Qgis.Info,
        )
    ...
    def setup_connections(self):
        QgsMessageLog.logMessage(
            "Setting up connections", "GeestConfigWidget", level=Qgis.Info
        )
        for key, widgets in self.widgets.items():
            ...
            if radio:
                ...
                QgsMessageLog.logMessage(
                    f"Set up radio connection for {key}",
                    "GeestConfigWidget",
                    level=Qgis.Info,
                )
            ...
            if widget_type == "classify_poly_into_classes":
                QgsMessageLog.logMessage(
                    f"Setting up specific connections for widget_type: {widget_type} "
                    f"(key: {key})",
                    "GeestConfigWidget",
                    level=Qgis.Info,
                )
                if hasattr(widget, "selectionsChanged"):
                    ...
                    QgsMessageLog.logMessage(
                        f"Connected selectionsChanged signal for key: {key}",
                        "GeestConfigWidget",
                        level=Qgis.Info,
                    )
            ...
            if widget:
                QgsMessageLog.logMessage(
                    f"Setting up connection for widget type: {type(widget).__name__} for "
                    f"key: {key}",
                    "GeestConfigWidget",
                    level=Qgis.Info,
                )
                if isinstance(widget, QgsMapLayerComboBox):
                    ...
                    def update_fields(layer):
                        QgsMessageLog.logMessage(
                            f"[setup_connections] populate_field_selector called for"
    ...
    def populate_field_selector(layer, field_selector):
        if isinstance(layer, QgsVectorLayer):
            ...
            QgsMessageLog.logMessage(
                f"Invalid layer type for populating field selector: {type(layer)}",
                "GeestConfigWidget",
                level=Qgis.Warning,
            )
    ...
    def update_polygon_layer_and_field(self, key, layer, field):
        QgsMessageLog.logMessage(
            f"update_polygon_layer_and_field called for {key}",
            "GeestConfigWidget",
            level=Qgis.Info,
        )
        QgsMessageLog.logMessage(
        ...
        QgsMessageLog.logMessage(
            f"Field: {field}", "GeestConfigWidget", level=Qgis.Info
        )
        ...
        if layer and isinstance(layer, QgsVectorLayer) and field:
            ...
            QgsMessageLog.logMessage(
                f"Layer URI: {uri}", "GeestConfigWidget", level=Qgis.Info
            )
            decoded = QgsProviderRegistry.instance().decodeUri(provider_key, uri)
            QgsMessageLog.logMessage(
                f"Decoded URI: {decoded}", "GeestConfigWidget", level=Qgis.Info
            )
            ...
            if path:
                value = f"{path};{field}"
                QgsMessageLog.logMessage(
                    f"Setting {key} to {value}", "GeestConfigWidget", level=Qgis.Info
                )
            ...
            QgsMessageLog.logMessage(
                f"No valid layer or field selected for {key}",
                "GeestConfigWidget",
                level=Qgis.Warning,
            )
            self.modified_config[key] = ""
    ...
    def update_classify_poly_config(self, key):
        QgsMessageLog.logMessage(
            f"update_classify_poly_config called for {key}",
            "GeestConfigWidget",
            level=Qgis.Info,
        )
        ...
        if widget and widget.property("widget_type") == "classify_poly_into_classes":
            ...
            if layer and field:
                ...
                QgsMessageLog.logMessage(
                    f"No layer or field selected for {key}",
                    "GeestConfigWidget",
                    level=Qgis.Warning,
                )
                self.modified_config[key] = ""
            ...
            QgsMessageLog.logMessage(
                f"Widget for {key} is not a ClassifyPolyIntoClassesWidget",
                "GeestConfigWidget",
                level=Qgis.Warning,
            )
            self.modified_config[key] = ""
    ...
    def update_layer_path(self, key, layer):
        QgsMessageLog.logMessage(
            f"update_layer_path called for {key}", "GeestConfigWidget", level=Qgis.Info
        )
        if layer:
            provider_key = layer.providerType()
            uri = layer.dataProvider().dataSourceUri()
            ...
            if path:
                QgsMessageLog.logMessage(
                    f"Path found: {path}", "GeestConfigWidget", level=Qgis.Info
                )
            ...
            QgsMessageLog.logMessage(
                f"No layer selected for {key}", "GeestConfigWidget", level=Qgis.Warning
            )
            self.update_sub_widget_state(key, None)

    def handle_option_change(self, option, checked):
        QgsMessageLog.logMessage(
            f"handle_option_change called for {option}, checked={checked}",
            "GeestConfigWidget",
            level=Qgis.Info,
        )
        if checked:
            for key, widgets in self.widgets.items():
                ...
                if key == option:
                    if widget is None:
                        ...
                        QgsMessageLog.logMessage(
                            f"Handling polygon_layer_with_field_selector for {key}",
                            "GeestConfigWidget",
                            level=Qgis.Info,
                        )
                        layer, field = widget.get_selections()
                        ...
                        QgsMessageLog.logMessage(
                            f"Handling QgsMapLayerComboBox for {key}",
                            "GeestConfigWidget",
                            level=Qgis.Info,
                        )
                        ...
                        QgsMessageLog.logMessage(
                            f"Handling multibuffer for {key}",
                            "GeestConfigWidget",
                            level=Qgis.Info,
                        )
                        ...
                        QgsMessageLog.logMessage(
                            f"Handling ClassifyPolyIntoClassesWidget for {key}",
                            "GeestConfigWidget",
                            level=Qgis.Info,
                        )
                        ...
                        QgsMessageLog.logMessage(
                            f"Setting {key} to 1", "GeestConfigWidget", level=Qgis.Info
                        )
                        self.modified_config[key] = 1
                else:
                    QgsMessageLog.logMessage(
                        f"Setting {key} to 0", "GeestConfigWidget", level=Qgis.Info
                    )
                    self.modified_config[key] = 0
        else:
            QgsMessageLog.logMessage(
                f"Setting {option} to 0 (unchecked)",
                "GeestConfigWidget",
                level=Qgis.Info,
            )
            self.modified_config[option] = 0
        QgsMessageLog.logMessage(
            f"Modified config after handle_option_change: {self.modified_config}",
            "GeestConfigWidget",
            level=Qgis.Info,
        )
        self.stateChanged.emit(self.get_state())
