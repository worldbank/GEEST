...
class RasterLayerWorkflow(WorkflowBase):
    ...
    def execute(self):
        ...
        """
        QgsMessageLog.logMessage("Executing spatial analysis workflow", 'Custom Workflows', Qgis.Info)

        steps = 10
        for i in range(steps):
            if self._feedback.isCanceled():
                QgsMessageLog.logMessage("Spatial analysis workflow canceled.", 'Custom Workflows', Qgis.Warning)
                return False
        ...
        QgsMessageLog.logMessage("Spatial analysis workflow completed", 'Custom Workflows', Qgis.Info)
        return True
